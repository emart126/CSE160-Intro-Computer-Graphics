...
function newDrawTriangle3DUVNormal(allVertices) {
  // Initialize buffer if needed
  if (g_vBuffer == null) {
    initializeTriangle3DUVNormal(allVertices.BYTES_PER_ELEMENT);
  }

  var n = allVertices.length / 11; // The number of vertices
...
function initializeTriangle3DUVNormal(FSIZE) {
  // === Create a buffer object for each vertex ===
  ...
  // === UV attribute ===
  gl.vertexAttribPointer(a_UV, 2, gl.FLOAT, false, FSIZE*11, FSIZE*3);
  gl.enableVertexAttribArray(a_UV);

  // === Color attribute ===
  gl.vertexAttribPointer(a_Color, 3, gl.FLOAT, false, FSIZE*11, FSIZE*5);
  gl.enableVertexAttribArray(a_Color);
